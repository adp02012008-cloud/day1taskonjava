    ...
    function applyDisc(){
      ...
      showTotal();
      if (discount) {alert('30% Discount Applied!');
      }else{
    ...
    function pay(){
      ...
      if(user!=='user' || pass!=='1234'){alert('Invalid username or password');return;}
      const success=Math.random()<0.9;
      if(success){
        for(let k in cart){stock[k]-=cart[k].qty;if(stock[k]<0)stock[k]=0;document.getElementById('stock-'+k).textContent=stock[k];}
